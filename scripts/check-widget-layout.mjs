...
/**
 * Impede que o layout do widget use algo que o RemoteViews não sabe inflar.
 * Isso não dá erro de compilação: o widget simplesmente falha na hora de ser
 * adicionado à tela inicial ("não foi possível adicionar o widget"), e só se
 * descobre no aparelho.
 *
 * A lista é a das classes anotadas com @RemoteView no framework, para minSdk 23.
 *
...
const LAYOUTS = ["android/app/src/main/res/layout/widget_schedule.xml"]
...
for (const path of LAYOUTS) {
  ...
  const tags = [...xml.matchAll(/<([A-Za-z][\w.]*)/g)].map((match) => match[1])
